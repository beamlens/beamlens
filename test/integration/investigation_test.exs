defmodule Beamlens.Integration.InvestigationTest do
  ...
  describe "Agent.investigate/2" do
    ...
    test "returns :no_alerts when given empty list" do
      assert {:ok, :no_alerts} = Agent.investigate([])
    end

    test "investigates single alert and returns health analysis" do
      alert = build_alert(:warning, "Memory usage elevated at 85%")

      {:ok, analysis} = Agent.investigate([alert])
    ...
    test "investigates multiple alerts and correlates findings" do
      alerts = [
    ...
    test "first event is watcher_alerts, not snapshot" do
    ...
    test "watcher_alerts event contains alert data" do
    ...
    test "excludes JudgeCall events when judge is disabled" do
      ...
      {:ok, analysis} = Agent.investigate([alert], judge: false)
